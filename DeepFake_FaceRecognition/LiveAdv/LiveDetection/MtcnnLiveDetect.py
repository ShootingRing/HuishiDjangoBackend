...
import time
import logging
from DeepFake_FaceRecognition.LiveAdv.LiveDetection.net.network import Model
...
def pre_precessing(args, name):
    ...
    print("视频流读取中...")
    # camera = cv2.VideoCapture('G:/PycharmProjects/djangoProjectVueProject/video/stream.mp4')
    camera = cv2.VideoCapture('G:/PycharmProjects/djangoProjectVueProject/video/streamTest.mp4')
    ...
    if args.video:
        ...
        loc = 'results/{}.mp4'.format(name)
        out = cv2.VideoWriter(loc, fourcc, int(fps), (1920, 1080))

    return net, model, camera, out


def main(args, name):
    net, model, camera, out = pre_precessing(args, name)
    # 模型正向推理
    # print('正向推理进行中...')
        ...
        for i in range(0, detections.shape[2]):
            ...
            if confidence > args.face_confidence:
                ...
                if len(face) != int(0):
                    ...
                    with open("results/MtcnnliveDetect_data.txt", "w") as f:
                        f.write(str(result))  # 自带文件关闭功能，不需要再写f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-name', dest='client_name', default="Liaoxin", type=str)
    ...
    parser.add_argument('--model_path', default=r'model/modelface.pt', help='活体检测模型路径')
